...
final _cliente = Provider<SupabaseClient>(
...
final metricasRetencionProvider = FutureProvider.autoDispose
    .family<({int dominados, int enCurso, int necesitaRepaso, int sinEvaluar,
        int total}),
  ...
  final enCurso = materiales
      .where((m) => m.estadoDominio == 'en_progreso')
      .length;
  final necesitaRepaso = materiales
      .where((m) => m.estadoDominio == 'necesita_repaso')
      .length;
  final sinEvaluar =
