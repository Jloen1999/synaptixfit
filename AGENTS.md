...
- **`app/lib/core/sync/`** — SyncHub (Fase 2): `DominioEvento` enum (11 eventos: planGuardado, bloqueEstudioCompletado, bloqueEstudioDesmarcado, sesionCompletada, sesionDesmarcada, checkInRealizado, entregaCompletada, retoCompletado, pomodoroCompletado, xpOtorgado, practicaCompletada), `EventoPayload` DTO con `horarioId`, `SyncHub` con mapa de invalidaciones simétricas por evento (ida y vuelta invalidan mismos providers neurofisiológicos + retos + timeline + dashboard), `syncHubProvider` (Riverpod Provider).
