...
- **`app/lib/shared/models/db_models.dart`** — all DB model classes (~2715 lines, incl. `MaterialEstudioDb` con SM-2, `BancoPreguntasDb`, `PreguntaDb`, `IntentoPreguntaDb`, `CargaAcademicaSemanalDb`, `AsignaturaUsuarioSemestreDb`, `EjercicioDb` con `valorMet`, `SeleccionEjercicioDb` con `duracionObjetivoSegundos`/`duracionRealSegundos`)
...
- **`app/lib/features/academico/`** (Sistema de Repaso Espaciado SM-2) — `sm2_calculator.dart` (algoritmo SM-2: facilidad, intervalo, estadoDominio), `practica_screen.dart` (ruta `/academico/practica/:materialId`, preguntas secuenciales con feedback inmediato, modal de autoevaluación 3 niveles, inyección de repaso en timeline vía `horarios_academicos` con `tipo_actividad='repaso'`, `es_fijo=false`), `practica_provider.dart` (`practicaRepositoryProvider`, `bancoPreguntasProvider.family`), `materiales_estudio_provider.dart` (`materialesEstudioRepositoryProvider`, `materialesAsignaturaProvider.family`)
...
- **`app/lib/core/sync/`** — SyncHub (Fase 2): `DominioEvento` enum (9 eventos: planGuardado, bloqueEstudioCompletado, sesionCompletada, checkInRealizado, entregaCompletada, retoCompletado, pomodoroCompletado, xpOtorgado, practicaCompletada), `EventoPayload` DTO, `SyncHub` con mapa de invalidaciones por evento, `syncHubProvider` (Riverpod Provider).
