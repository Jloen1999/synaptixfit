-- ============================================================================
-- Migración 20260701000031: Fix RPCs — columnas correctas en export/anonymize
-- ============================================================================

BEGIN;

-- Corregir anonymize_user: eliminar registros_repaso_srs (no tiene usuario_id)
-- y añadir materiales_estudio (FK → registros_repaso_srs cascadea)
...
BEGIN
  ...
  UPDATE public.sesiones_registradas SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  GET DIAGNOSTICS v_count = ROW_COUNT; v_total_anonimizados := v_total_anonimizados + v_count;
  UPDATE public.carga_academica_semanal SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  GET DIAGNOSTICS v_count = ROW_COUNT; v_total_anonimizados := v_total_anonimizados + v_count;
  UPDATE public.horarios_academicos SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  GET DIAGNOSTICS v_count = ROW_COUNT; v_total_anonimizados := v_total_anonimizados + v_count;
  UPDATE public.estado_cognitivo_usuario SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  UPDATE public.estado_regulacion_cruzada SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  UPDATE public.registros_carga_fisica SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  UPDATE public.estado_diario_usuario SET usuario_id = v_anon_id WHERE usuario_id = p_usuario_id;
  ...
  GET DIAGNOSTICS v_count = ROW_COUNT; v_total_eliminados := v_total_eliminados + v_count;
  ...
  VALUES (v_admin_id, 'anonimizar_usuario', 'usuarios', p_usuario_id,
    jsonb_build_object('email_anonimizado', v_email, 'anon_id_asignado', v_anon_id,
      'registros_anonimizados', v_total_anonimizados, 'registros_eliminados', v_total_eliminados));

  RETURN jsonb_build_object('success', true, 'email', v_email, 'anon_id', v_anon_id,
...
-- Corregir exportar_datos_usuario: historial_peso.registrado_en + quitar registros_repaso_srs
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'horarios_academicos', (SELECT jsonb_agg(row_to_json(h) ORDER BY h.fecha DESC, h.hora_inicio)
...
-- Corregir delete_user: añadir materiales_estudio, quitar registros_repaso_srs
...
DECLARE
  ...
  v_user_email TEXT;
BEGIN
  SELECT u.id INTO v_admin_id FROM public.usuarios u WHERE u.id = auth.uid() AND u.rol = 'admin';
  IF v_admin_id IS NULL THEN RAISE EXCEPTION 'Solo administradores pueden eliminar usuarios'; END IF;
  IF v_admin_id = p_usuario_id THEN RAISE EXCEPTION 'No puedes eliminar tu propio usuario'; END IF;
  SELECT email INTO v_user_email FROM public.usuarios WHERE id = p_usuario_id;
  IF v_user_email IS NULL THEN RAISE EXCEPTION 'Usuario no encontrado'; END IF;
